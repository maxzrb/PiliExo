...
- 更新时间：2026-09-20 15:22 (+08:00)
- 工作分支：`merge/upstream-2.1.4-20260913`（跟踪 `origin/merge/upstream-2.1.4-20260913`）
- 分析基线：`43d7bde9d`（`v26.9.15.1` 发布提交，annotated tag 已推送）
- 发布状态：`v26.9.15.1` 已发布到 GitHub Release 和 ModelScope，应用版本 `26.9.15+1`，Android `versionCode=22`。
- 当前工作：已审查 PiliPlus 自 2.1.4 同步边界 `ce17223a8` 至 `upstream/main@48f075758` 的 40 个新增提交；尚未合并代码，已形成分级候选与暂缓清单。
- 当前验证：Flutter 3.47.4 全量测试 64/64 通过；Analyze 无 error/warning（54 条既有 info）；双 ABI Release 构建、包名/版本/ABI/FFmpeg、R8 映射、V2 正式签名、GitHub digest 和 ModelScope 长度/哈希均通过。Android 原生单测在首次冷缓存编译期间按用户要求停止，未连接 Android 真机。
- 目标：PiliExo 仅 Android 在线 UGC/PGC HDR 使用 Media3 原生 SurfaceView；SDR、直播和离线保持 mpv；应用包名独立为 com.maxzrb.piliexo，外观磨砂效果可配置；暂不接入 Android Kyant 液态玻璃。Android `versionCode` 按正式 Release 全局递增，保留 `vYY.M.D.N` 标签格式；项目锁定 Flutter 3.47.4；当前正式版本为 `v26.9.15.1`。

## 待同步候选（审查至 2026-09-20）

- 上游审查边界：`ce17223a8..48f075758`，共 40 个提交；远端 `upstream/main` 与 `git ls-remote` 一致。当前仅审查，未执行 cherry-pick、版本变更或构建。
- 第一优先级（服务端/API 兼容）：`7399f3507` + `08a9f5509`（App 推荐请求参数与新卡片解析）、`c876e5d8b`（热门/每周必看 API 请求头）、`20b1cb5c0`（搜索建议迁移到 WBI 新接口，依赖 `11e02b866` 的搜索控制器上下文）。
- 第二优先级（低风险修复/体验）：`11e02b866`（搜索框直开链接、Android 外部应用选择器、patch 文件固定 LF、历史分页空结果处理等）、`f1a41ed0f`（多 P 视频分享保留分 P 与播放进度）、`faf49b355`（直播接口账户选择修复）、`8b6abc07d` + `89fe1755f`（电竞比分与标题字段适配）、`92c455be7`（自定义主题种子色）、`2408a7f4d`（搜索历史单行省略）。
- 可选但建议人工适配：`df86de3ec`（画中画中收到 SC 时避免全屏 SC 层处理）、`41ccd5801` + `94e413ac0` + `1cf440ed9`（后台播放/系统媒体状态与 wakelock 时序修复，和 PiliExo 播放器定制重叠）、`7afc32cf8` + `92f53a997`（SC 保存为图片，依赖前序直播/截图重构）、`41fe78223`（不再提前传分 P 标题，和本地已修改的入口重叠）。
- 暂缓：Linux/Windows 专属提交；`00cb572cc`/`ee148aa64` media-kit 升级与部分回退、`9d3e8b626` 依赖升级；`c61777333`/`5d02a625d` 选择文本搜索与图片弹窗链路（最终代码会忽略调用方传入的自定义 `contextMenuBuilder`）；`b6908e515`/`b0e7e4eb1` 图片批量保存重构（同时改变取消下载、缓存文件移动与保存路径语义）；`2b0ffa12f`/`48f075758` tooltip 重构和 `d1d5c323c` 大型杂项重构，均与本地定制重叠且不宜整提交搬运。
- 可移植性预检：`11e02b866`、`8b6abc07d`、`f1a41ed0f`、`7399f3507`、`08a9f5509`、`faf49b355`、`df86de3ec`、`92c455be7`、`c876e5d8b`、`2408a7f4d` 可直接通过 `git apply --check`；`20b1cb5c0`、`7afc32cf8`、`6755d0942` 需要前序提交或人工适配。
...
- 已在 `C:\Codex Program\piliexo` 从 `origin/main` 建立工作区；当前为 `main` 分支、HEAD `c01f61463`，仓库源文件未改动。
- 已固定 Flutter 3.47.2（Dart 3.13.2）至 `C:\tools\flutter-3.47.2\flutter`，Android SDK 至 `C:\Android\Sdk`，Java 至 Microsoft OpenJDK 17.0.20.1+1；Android 37.0、Build Tools 37.0.0、NDK 28.2.13676358 和许可证均可用，桌面目标按项目定位关闭。
- 已按项目脚本应用 24 个 Flutter framework 补丁，并按同一补丁集应用 `material_ui` Android 补丁；依赖使用 `flutter pub get --offline` 完成。Windows 长路径导致的 `flutter_inappwebview_android` Git 缓存缺失文件已补齐，媒体 JAR 已按 MD5 校验后缓存。
- 验证通过：Flutter 全量测试 46/46；`flutter analyze --no-pub --no-fatal-infos` 无 error（51 条既有 info）；`flutter build apk --debug --no-pub` 成功；Debug APK 为 `build/app/outputs/flutter-apk/app-debug.apk`，154,122,291 bytes，SHA-256 为 `970badf32710f243f5fd40ed96811ec99222966b1c693d7702d5b77d62a6ec66`；未连接 Android 真机。
- 已审计仓库和用户备份目录 `C:\Users\maxzr\Downloads\piliexo`，均未找到正式签名文件。项目约定文件名为 `android/piliexo-release.jks`，配置为 `android/key.properties`，别名为 `piliexo`；未生成新密钥、未执行正式 Release 构建。
- 本轮未修改版本号；仅更新本状态记录和 `version/工作进度.md`，未自动提交。构建产物、SDK 和 Pub/Gradle 缓存均在项目外或被忽略。

## 2026-09-02 12:35

- 由于仓库和用户备份中均没有旧正式签名密钥，已在 `android/piliexo-release.jks` 生成新的 JKS release 密钥；别名为 `piliexo`，RSA 4096，配套密码配置保存在 `android/key.properties`。两个文件均被 `.gitignore` 忽略，未把密码写入交接记录；需由用户上传至网盘私人空间或加密备份，不要公开分享。
- 新签名与旧签名不同，同包名的旧签名安装包不能直接覆盖升级；正式分发新签名包前需安排卸载重装或数据迁移。
- 修复播放器洞察带宽估计偶尔显示的问题：播放器缓存空闲时返回的 `0` 不再覆盖最近一次有效带宽估计；首次只有零值时仍保持未测量，非法值也不会清空已有结果。新增零值回归测试。
- 验证通过：Flutter 全量测试 47/47；Analyze 无 error（51 条既有 info）；release APK `build/app/outputs/flutter-apk/app-release.apk` 构建成功，大小 72.0 MB；`apksigner` V2 校验通过且证书与新 JKS 指纹一致。未连接 Android 真机，未发布新版本，未自动提交。

## 2026-09-02 13:03

- 已添加并 fetch PiliPlus 上游远端 `https://github.com/bggRGjQaUbCoE/PiliPlus.git`；`upstream/main` 为 `adc9a4b94`，相对共同基线 `44680b8a4` 有 14 个上游独有提交。`git merge-tree --write-tree` 显示整批文本合并无冲突，但未直接合入当前分支。
- 隔离验证通过的候选：账户删除防回写、请求 Cookie 绑定、WBI 视频详情接口、登录切换、弹幕下载并发限制（`6d3541dcd`、`628031d4d`、`f3f59775f`、`95d299d30`、`315068bbe`）全量测试 48/48，Analyze 无 error；关注分组排序 `c7458b1f3` 测试 46/46；音频均衡 `03dcaa746` + `4b91b4001` 测试 46/46；账户优化 `adc9a4b94` 叠加账户组测试 48/48；发布面板 `66d1f7d62` 测试 46/46。
- 整批 14 个提交隔离测试未通过：字体提交 `828de30e9` 与 PiliExo 已有字体迁移代码发生 `String?` 到 `String` 的编译错误（`lib/utils/font_utils.dart:46`）；因此整批合并不安全。`adbf6f3f5` 的 Android provider authority 已由 PiliExo 现有 `${applicationId}` 配置覆盖，无需重复合并；iOS 元数据和 CI workflow 变更不适用于当前 Android 发布目标。
- 当前分支仍为 `main`、HEAD `c01f61463`；工作区保留 4 个未提交文件（本轮带宽修复/测试及两份 HandShake 记录），未合入任何上游源码，未改版本号。上游审查用临时 worktree 已清理；建议先提交当前本地修复，再按候选分批 cherry-pick 并做真机回归。

## 2026-09-02 14:18

- 已创建回退分支 `backup/pre-upstream-selective-20260902`，保护本地改动后按依赖顺序无冲突 cherry-pick PiliPlus 上游 9 个提交：`03dcaa746`、`4b91b4001`、`6d3541dcd`、`628031d4d`、`f3f59775f`、`95d299d30`、`315068bbe`、`c7458b1f3`、`adc9a4b94`；未合入 `828de30e9`、`66d1f7d62` 及不适用于 Android 目标的提交。
- cherry-pick 后且恢复本地带宽修复后的最终验证通过：Flutter 全量测试 49/49，Analyze 无 error（51 条既有 info），`git diff --check` 通过；本地 stash 已干净弹回并删除，没有覆盖本地修改。
- 当前 Debug APK 构建尝试因 Gradle 依赖 HTTPS 下载无响应而中止，未发现代码编译错误；已确认没有残留 Java/Gradle 构建进程。此前本地 release APK 和新密钥 V2 签名验证仍有效。
- `828de30e9` 有中等价值，提供 `FontWeight` v2 存储/迁移和字体设置整理，但直接合并会在 PiliExo 现有 `font_utils.dart` 触发 `String?` 到 `String` 编译错误；它与播放器问题无关，建议后续单独适配后再合入。`66d1f7d62` 只有发布/评论输入面板的焦点和键盘体验收益，涉及 10 个 UI 文件，缺少真机键盘回归，当前 Android 播放器目标下暂缓。
- 当前 `main` 比 `origin/main` 超前 9 个提交，工作区保留 `docs/codex/STATUS.md`、`lib/plugin/pl_player/utils/mpv_bandwidth.dart`、`test/plugin/pl_player/mpv_bandwidth_test.dart` 和 `version/工作进度.md` 4 个未提交文件；未改版本号、未自动 commit/push、未连接 Android 真机。后续应重点回归登录切换/多账号 Cookie、视频详情、音频播放、弹幕下载、关注排序及播放器洞察。

## 2026-09-02 14:37

- 在回退分支 `backup/pre-font-publish-adaptation-20260902` 保护下，无冲突合入 PiliPlus `828de30e9`（字体字重 v2）和 `66d1f7d62`（发布/评论面板优化）；当前对应提交为 `51d744d16`、`9573e5dcc`。
- 字体适配提交 `630a0cfc1` 修复 PiliExo 自定义字体旧索引迁移的可空类型问题，保留旧版字体文件迁移，并将 v1 字重值迁移到 `appFontWeightV2`；异常索引会限制到 `w100`–`w900` 有效范围。新增 `test/utils/font_weight_setting_test.dart`，专项测试 3/3 通过；`463dbd419` 清理了新增的 FontWeight 弃用提示。
...
- `[修复]` 修复 HDR 的 4:3、16:9 模式只把画面缩小到目标区域的问题：保留外层 `AspectRatio` 目标视口，并让 Media3 原生画面用 `fill` 填满该视口，实际应用目标显示比例。
- `[修改]` 更新 HDR 固定比例模式的原生 resize mode 映射，避免 16:9 视频在 4:3 视口中继续按源比例缩小显示。
- 已按正式发布流程完成 `v26.9.6.3`，发布提交 `eb5b4d06afb983dd56c390b621516dc25309ab0b` 已推送至 `maxzrb/PiliExo/main`，annotated tag 已推送。
- Flutter 全量测试 62/62、Analyze 无 error/warning（55 条 info）、双 ABI Release 构建、包信息/ABI/FFmpeg、apksigner V2 正式签名及 `git diff --check` 均通过。
- arm64-v8a APK 为 `31,695,249` bytes，SHA-256 `7af8a452ad17dbf42ee3abb0c40cf91988eb1779f60a78ecd0b72117fd4fae42`；armeabi-v7a APK 为 `31,575,261` bytes，SHA-256 `160992521f0cc0cd689cc540c8358f675b7271ce0e9d18c58b44fad53c60a24f`；清单位于 `dist/release/v26.9.6.3/SHA256SUMS.txt`。
...
- 已按正式流程完成 `v26.9.15.1`：发布提交 `43d7bde9de4591a06a6aa6de4858827e9289ca21`，应用版本 `26.9.15+1`，Android `versionCode=22`；分支与 annotated tag 均已推送。
- 本版统一播放器洞察的常见音视频编码名称，并修复首次进入 HDR 时 EventChannel 连接前格式/解码器事件丢失、洞察偶发只显示分辨率的问题；原始 Codec String 继续保留。
- Flutter 3.47.4 全量测试 64/64 通过；Analyze 无 error/warning（54 条既有 info）；两个 Release APK 的包名、版本、单 ABI、`libffmpegJNI.so`、R8 `FfmpegAudioRenderer`、V2 正式签名均通过，证书 SHA-256 与上一版一致。Android 原生单测因冷缓存 Google Maven 下载耗时较长，按用户要求停止并直接完成正式 Release 构建。
- arm64-v8a APK：31,550,568 bytes，SHA-256 `3c51cc401c3a094dd3b026096b99d9fb254dbb535b2abb7f10795e02a4c81070`；armeabi-v7a APK：31,472,333 bytes，SHA-256 `433ab2f8169460844cfef545e02ff5851f4a89d7788b03251d8bdc1c77f746ef`。
- GitHub Release：<https://github.com/maxzrb/PiliExo/releases/tag/v26.9.15.1>；三个资产均为 uploaded，GitHub digest/大小与本地一致。ModelScope `AerithDream/PiliExo` 已同步至 `releases/v26.9.15.1/`，远端 HTTP 200、长度与 `X-Linked-ETag` 均校验通过。
- 对外 Release notes 与版本迭代记录未写 `versionCode`、ABI 数量等发布元数据；该约定已写入发布流程。未连接 Android 真机。

## 2026-09-20 15:22

- 从干净的 `merge/upstream-2.1.4-20260913` 分支执行 `git pull --ff-only`，结果为 Already up to date；抓取并核对 PiliPlus `upstream/main@48f075758`，自上次 2.1.4 边界 `ce17223a8` 后共有 40 个提交。
- 逐提交检查标题、受影响文件和关键 diff，并与 PiliExo 上次同步后的本地改动交叉比对；40 个提交涉及 90 个文件，其中 13 个文件与本地发布后定制重叠。
- 建议优先同步 App 推荐参数/解析、热门 API 请求头和搜索建议 WBI 新接口；其次同步外链选择器/搜索路由、多 P 分享、直播账户、电竞字段、自定义主题色和搜索历史布局。
- media-kit 升级后已被上游部分回退，播放器状态链路又与 PiliExo 的 Media3、洞察、震动和 wakelock 定制重叠，本轮不建议整批合并；Linux/Windows 专属提交不纳入 Android 发布范围。
- 对第一、第二优先级候选做无写入 `git apply --check` 预检：除需要前序上下文的搜索建议提交外，其余主要候选可直接应用；本轮没有合并源码、修改版本号、运行测试或构建 APK。
- 本次只更新 HandShake 审查记录和中文工作进度；工作树因此包含两份文档改动，建议若准备实施同步，先以独立文档提交保存本轮审查结论。
